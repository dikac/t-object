...
import Map from "./map";
export default function MapCallbackFunction<Container extends Record<PropertyKey, Validator> = Record<PropertyKey, Validator>, Result extends Record<PropertyKey, Instance> = Record<PropertyKey, Instance>, ValidatableT extends Validatable = Validatable, MessageT = unknown>(validators: Container, map: (argument: RecordParameter<Container>, validators: Container) => Result, validation: (result: Result) => ValidatableT, message: (result: Result) => MessageT): Map<Container, Result, ValidatableT, MessageT>;
